  ...
  <div class="app-container">
    ...
    <div class="search-box">
      <el-form ref="searchFormRef" :model="searchForm" class="searchForm" :inline="true">
        ...
        <el-form-item label="处置建议" align="left">
          <el-select
            v-model="searchForm.suggestion"
            size="small"
            placeholder="请选择监测结果"
          >
            <el-option
        ...
        <el-form-item label="心电分析结果" align="left">
          <el-input
            v-model="searchForm.ecgResult"
            size="small"
            placeholder="请输入心电分析结果"
          ></el-input>
        </el-form-item>
        <el-form-item label="是否打印" align="left">
          <el-select
            class="w100"
        ...
        <el-form-item label="监测日期">
          <el-date-picker
            v-model="searchForm.monitorTime"
            size="small"
            type="daterange"
            align="right"
            unlink-panels
    ...
    <div style="text-align: right; margin-bottom: 10px">
      <el-tooltip
        class="item"
        effect="dark"
        content="注意：重置次数是指将选择日期内的打印次数重置为0"
        placement="top-start"
      >
        <el-button plain @click="resetPrintCount" size="small">重置打印次数</el-button>
      </el-tooltip>
    </div>
    <EleTable
      :data="list"
    ...
    >
      ...
      <el-table-column
        ...
        prop="doctorUserName"
      ></el-table-column>
      <el-table-column align="center" fixed="left" label="姓名" prop="patientUserName">
      ...
      <el-table-column align="center" label="审核人" prop="auditorName"></el-table-column>
      <el-table-column align="center" label="已打印次数" prop="printNumber">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作" fixed="right" width="180">
        <template slot-scope="scope">
          <div v-if="scope.row.isAnalysisIng === false">
            <el-button size="mini" @click="examineReport(scope.row)" type="primary"
              >查看报告</el-button
            >
            <!-- 打印按钮 只有审核完毕才能打印 -->
            <el-button
          ...
          <div v-else>
            <el-button size="mini" disabled plain>正在分析中</el-button>
          </div>
        </template>
      </el-table-column>
    </EleTable>
      ...
      <div class="print-container">
        <div class="container" id="printMe">
          ...
          <div class="analyse">
            ...
            <div class="flex margin resultWidth">
              <div>
                ...
                <span class="fw fz16" v-html="heartDetail.avg"></span>
                <span class="fw">bpm</span>
              </div>
              <div>
                <span class="fw">最高心率：</span>
                <span class="fw fz16" v-html="heartDetail.max"></span>
                <span class="fw">bpm</span>
              </div>
              <div>
                <span class="fw">最低心率：</span>
                <span class="fw fz16" v-html="heartDetail.min"></span>
                <span class="fw">bpm</span>
              </div>
            </div>
            <div class="flex resultWidth">
              <div>
                ...
                <span class="fw fz16" v-html="heartDetail.normalRate"></span>
                <span class="fw">%</span>
              </div>
              <div>
                <span class="fw">心率偏快：</span>
                <span class="fw fz16" v-html="heartDetail.heartbeatRate"></span>
                <span class="fw">%</span>
              </div>
              <div>
                <span class="fw">心率偏慢：</span>
                <span class="fw fz16" v-html="heartDetail.slowRate"></span>
                <span class="fw">%</span>
              </div>
            </div>
          </div>
          <div class="impression">
            ...
            <div class="ecgResultTz" v-html="heartDetail.ecgResultTz"></div>
          </div>
          <div class="result">
            <div class="fz14">心电分析结果：</div>
            <div class="fz11 result-text" v-html="heartDetail.ecgResult"></div>
            <div class="result-option">
              <div class="fw result-title">处置建议：</div>
              <div class="content" v-html="heartDetail.suggestion"></div>
            </div>
            <div class="result-option middle">
              <div class="fw result-title">原因分析：</div>
              <div class="content" v-html="heartDetail.abnorAnalysis"></div>
            </div>
            <div class="result-option">
              <div class="fw result-title">保健建议：</div>
              <div class="content" v-html="heartDetail.healthCareAdvice"></div>
            </div>
          </div>
          <!-- 底部 -->
          <div class="footer">
...
export default {
  ...
  methods: {
      ...
      httpAdminHeartRate.getHeartRate(searchForm).then(
        (res) => {
          ...
          handleDataList.forEach((item) => {
            if (item.length) {
              item.length = formatSeconds(JSON.parse(item.reportResult).body.data.length);
            }
    ...
    // 记录打印次数
    onPrint() {
      httpAdminHeartRate.putHeartRatePrint({ id: this.printId }).then((res) => {
        console.log(res);
      });
    },
    // 重置打印次数
